...
class ArbolAVL {
    ...
    private void balancear(int i) {
        ...
        if (fe > 1) {
            ...
            if (obtenerFactorEquilibrio(hijoIzq) >= 0) {
                System.out.println("Rotación Simple Derecha en nodo con ID: " + ids[i]);
                rotacionDerecha(i);
            } else {
                System.out.println("Rotación Doble Izquierda-Derecha en nodo con ID: " + ids[i]);
            ...
            if (obtenerFactorEquilibrio(hijoDer) <= 0) {
                System.out.println("Rotación Simple Izquierda en nodo con ID: " + ids[i]);
                rotacionIzquierda(i);
            } else {
                System.out.println("Rotación Doble Derecha-Izquierda en nodo con ID: " + ids[i]);
    ...
    public boolean buscar(int id) {
        System.out.print("Buscando ticket " + id + ": ");
        int indice = buscarRec(0, id);
        if (indice != -1) {
            System.out.println("Encontrado en índice de arreglo: " + indice);
            return true;
        } else {
    ...
    private int buscarRec(int i, int id) {
        if (i >= capacidad || !ocupado[i]) {
            return -1;
        }
        if (ids[i] == id) {
        ...
        if (id < ids[i]) {
            return buscarRec(2 * i + 1, id);
        }
        return buscarRec(2 * i + 2, id);
    }
    ...
    private void eliminarRec(int i, int id) {
        ...
        if (id < ids[i]) {
            ...
            if (!tieneIzq || !tieneDer) {
                ...
                if (hijoReal >= capacidad || !ocupado[hijoReal]) {
                    ocupado[i] = false;
                    ids[i] = 0;
                    alturas[i] = 0;
                } else {
                ...
                int minIndice = obtenerIndiceMinimo(hijoDer);
                ids[i] = ids[minIndice];
                eliminarRec(hijoDer, ids[minIndice]);
            }
        }
    ...
    private void inordenRec(int i) {
        if (i < capacidad && ocupado[i]) {
            inordenRec(2 * i + 1);
            System.out.print(ids[i] + "(FE:" + obtenerFactorEquilibrio(i) + ") ");
            inordenRec(2 * i + 2);
        }
    }

    public void mostrarArreglos() {
        System.out.print("Arreglo IDs:      [");
        ...
        System.out.print("Arreglo Factores: [");
        for (int i = 0; i < capacidad; i++) {
            System.out.print(ocupado[i] ? obtenerFactorEquilibrio(i) : "-");
            if (i < capacidad - 1) System.out.print(", ");
        }
        System.out.println("]");
        System.out.println("---------------------------------------");
