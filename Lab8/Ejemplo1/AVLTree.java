...
class AVLTree {
    ...
    private void balancear(int i) {
        ...
        if (fe > 1) {
            ...
            if (obtenerFactorEquilibrio(hijoIzq) >= 0) {
                System.out.println("-> Rotación Simple Derecha en código: " + ids[i]);
                rotacionDerecha(i);
            } else {
                System.out.println("-> Rotación Doble Izquierda-Derecha en código: " + ids[i]);
            ...
            if (obtenerFactorEquilibrio(hijoDer) <= 0) {
                System.out.println("-> Rotación Simple Izquierda en código: " + ids[i]);
                rotacionIzquierda(i);
            } else {
                System.out.println("-> Rotación Doble Derecha-Izquierda en código: " + ids[i]);
    ...
    public boolean buscar(int id) {
        return buscarRec(0, id);
    }

    private boolean buscarRec(int i, int id) {
        if (i >= capacidad || !ocupado[i]) {
            return false;
        }
        if (id == ids[i]) {
